  subroutine recip12as(elat,ec6,lgrad1,lgrad2)
!
!  Calculate reciprocal space contribution to region 1 - region 2a
!  interaction. Symmetry adapted version.
!
!  imode = 1 => defective region 1 - region 2a only
!
!   5/95 New algorithm to avoid cosine and sine recalc added for
!        cases where onepass won't fit
...
!
  if (lc6) then
    ...
    if (status/=0) call outofmemory('recip12as','ktrm6')
  endif
!**********************************
...
!
  if (lc6) then
    ...
    if (status/=0) call deallocate_error('recip12as','ktrm6')
  endif
!
!  Timing
!
