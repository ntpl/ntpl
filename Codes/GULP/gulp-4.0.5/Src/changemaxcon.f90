...
!
  ...
  call realloc(ncfix,maxcon,ierror)
  if (ierror.ne.0) call outofmemory('changemaxcon','ncfix')
  call realloc(ncvar,maxcon,ierror)
  if (ierror.ne.0) call outofmemory('changemaxcon','ncvar')
!
  return
  end
