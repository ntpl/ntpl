  subroutine connectwraptrial(ntrialatom,nptrtrialatom,ltrialatom)
!
...
!  of more than half of a cell. Version that operates only on
!  trial set of atoms.
!
!   1/08 Created from connectwrap
...
!
  do ic = 1,nconnect
    if (nconnectcfg(ic).eq.ncf.and.nconnectind(ic).gt.0) then
      i = n1connect(ic)
      j = n2connect(ic)
      if (ltrialatom(i).or.ltrialatom(j)) then
!
