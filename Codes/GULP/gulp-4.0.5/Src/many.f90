...
!
                  ...
                  do while (l.lt.numat.and.lnonzeroSij)
                    l = l + 1
                    lopl = (.not.lfreeze.or.lopf(nrelat(l)))
                    if (lopl) then
                      lxc = lxc + 3
                      lyc = lyc + 3
                      lzc = lzc + 3
...
!
                        ...
                        if (lstr) then
                          do kl = 1,nstrains
                            ks = nstrptr(kl)
                            wrk(kl) = wrk(kl) + partial%sa_drhotots(ks,np1)*ofct
                          enddo
                          if (latomicstress) then
                            if (.not.lfreeze) then
                              do kl = 1,nstrains
                                ks = nstrptr(kl)
                                atomicstress(kl,i) = atomicstress(kl,i) + sixth*partial%sa_drhotots(ks,np1)*ofct
                                atomicstress(kl,j) = atomicstress(kl,j) + sixth*partial%sa_drhotots(ks,np1)*ofct
                                atomicstress(kl,k) = atomicstress(kl,k) + sixth*partial%sa_drhotots(ks,np1)*ofct
                              enddo
                            else
                              do kl = 1,nstrains
                                ks = nstrptr(kl)
                                atomicstress(kl,i) = atomicstress(kl,i) + third*partial%sa_drhotots(ks,np1)*ofct
                                atomicstress(kl,j) = atomicstress(kl,j) + third*partial%sa_drhotots(ks,np1)*ofct
                                atomicstress(kl,k) = atomicstress(kl,k) + third*partial%sa_drhotots(ks,np1)*ofct
                              enddo
                            endif
                          endif
                        endif
...
!
          ...
          if (lgrad2) then
            ...
            enddo
            if (lstr) then
              deriv2s(1:21) = deriv2s(1:21) + (rscrhoi*drhototij2s(1:21) + rscrhoj*drhototji2s(1:21))*ofct
              deriv2m(1:6,1:3) = deriv2m(1:6,1:3) + (rscrhoi*drhototij2m(1:6,1:3) + rscrhoj*drhototji2m(1:6,1:3))*ofct
              ind = 0
              do kl = 1,6
                do km = 1,kl
                  ind = ind + 1
...
!*****************************
          if (lsg1.or.lgrad2) then
            ...
            do kl = 1,nstrains
              ks = nstrptr(kl)
              wrk(kl) = wrk(kl) + derivs(ks)*oij
            enddo
            if (latomicstress) then
              if (.not.lfreeze) then
                do kl = 1,nstrains
                  ks = nstrptr(kl)
                  atomicstress(kl,i) = atomicstress(kl,i) + 0.25_dp*derivs(ks)*oij
                  atomicstress(kl,j) = atomicstress(kl,j) + 0.25_dp*derivs(ks)*oij
                enddo
              else
                do kl = 1,nstrains
                  ks = nstrptr(kl)
                  atomicstress(kl,i) = atomicstress(kl,i) + 0.5_dp*derivs(ks)*oij
                  atomicstress(kl,j) = atomicstress(kl,j) + 0.5_dp*derivs(ks)*oij
                enddo
              endif
            endif
!******************************
...
!
                do mp = 1,npote
                  if (nptype(mp).eq.19) then
                    lvalidik = .false.
                    if (rik.gt.rpot2(mp).and.rik.le.rpot(mp)) then
                      if (nati.eq.nspec1(mp).and.natk.eq.nspec2(mp)) then
                        if (ntypi.eq.nptyp1(mp).or.nptyp1(mp).eq.0) then
                          if (ntypk.eq.nptyp2(mp).or.nptyp2(mp).eq.0) lvalidik = .true.
                        endif
                      elseif (nati.eq.nspec2(mp).and.natk.eq.nspec1(mp)) then
                        if (ntypi.eq.nptyp2(mp).or.nptyp2(mp).eq.0) then
                          if (ntypk.eq.nptyp1(mp).or.nptyp1(mp).eq.0) lvalidik = .true.
                        endif
                      endif
                      if (lvalidik) then
!**********************************
...
!  Compute screening function
!
                                call meamscreen(rik2,vectorpair%distance_pair1(lvec),vectorpair%distance_pair2(lvec),Silk, &
                                                dSilkdr,lpartial,.true.)
!
!  If screening function contribution is 0, then no need to continue for this pair
!
...
!
                                    ...
                                    partialik%sa_atom(npartial) = l
                                    partialik%sa_xik(npartial) = vectorpair%xvector_pair1(lvec)
                                    partialik%sa_yik(npartial) = vectorpair%yvector_pair1(lvec)
                                    partialik%sa_zik(npartial) = vectorpair%zvector_pair1(lvec)
                                    partialik%sa_xjk(npartial) = vectorpair%xvector_pair2(lvec)
                                    partialik%sa_yjk(npartial) = vectorpair%yvector_pair2(lvec)
                                    partialik%sa_zjk(npartial) = vectorpair%zvector_pair2(lvec)
                                    partialik%sa_Sikj(npartial) = Silk
                                    partialik%sa_dSikjdr(1:3,npartial) = dSilkdr(1:3)
                                  endif
                                endif
!
!  End loop over images of possible screening atoms
                ...
                do mp = 1,npote
                  if (nptype(mp).eq.19) then
                    ...
                    if (rjk.gt.rpot2(mp).and.rjk.le.rpot(mp)) then
                      if (natj.eq.nspec1(mp).and.natk.eq.nspec2(mp)) then
                        if (ntypj.eq.nptyp1(mp).or.nptyp1(mp).eq.0) then
                          if (ntypk.eq.nptyp2(mp).or.nptyp2(mp).eq.0) lvalidjk = .true.
                        endif
                      elseif (natj.eq.nspec2(mp).and.natk.eq.nspec1(mp)) then
                        if (ntypj.eq.nptyp2(mp).or.nptyp2(mp).eq.0) then
                          if (ntypk.eq.nptyp1(mp).or.nptyp1(mp).eq.0) lvalidjk = .true.
                        endif
                      endif
                      if (lvalidjk) then
!**********************************
...
!  Compute screening function
!
                                call meamscreen(rjk2,vectorpair%distance_pair1(lvec),vectorpair%distance_pair2(lvec),Sjlk, &
                                                dSjlkdr,lpartial,.true.)
!
!  If screening function contribution is 0, then no need to continue for this pair
!
...
!
                                    ...
                                    partialjk%sa_atom(npartial) = l
                                    partialjk%sa_xik(npartial) = vectorpair%xvector_pair1(lvec)
                                    partialjk%sa_yik(npartial) = vectorpair%yvector_pair1(lvec)
                                    partialjk%sa_zik(npartial) = vectorpair%zvector_pair1(lvec)
                                    partialjk%sa_xjk(npartial) = vectorpair%xvector_pair2(lvec)
                                    partialjk%sa_yjk(npartial) = vectorpair%yvector_pair2(lvec)
                                    partialjk%sa_zjk(npartial) = vectorpair%zvector_pair2(lvec)
                                    partialjk%sa_Sikj(npartial) = Sjlk
                                    partialjk%sa_dSikjdr(1:3,npartial) = dSjlkdr(1:3)
                                  endif
                                endif
!
!  End loop over images of possible screening atoms
...
!
                if (lMEAM) then
                  ...
                  do kl = 1,nstrains
                    ...
                    derv3(jx,kl) = derv3(jx,kl) + dt1*drhototiks(kl)*drhototij(1) + dt2*drhototjks(kl)*drhototji(1)
                    derv3(jy,kl) = derv3(jy,kl) + dt1*drhototiks(kl)*drhototij(2) + dt2*drhototjks(kl)*drhototji(2)
                    derv3(jz,kl) = derv3(jz,kl) + dt1*drhototiks(kl)*drhototij(3) + dt2*drhototjks(kl)*drhototji(3)
!
                    ...
                    derv3(jx,kl) = derv3(jx,kl) + dt3*drhototijk2m(kl,1) + dt4*drhototjik2m(kl,1)
                    derv3(jy,kl) = derv3(jy,kl) + dt3*drhototijk2m(kl,2) + dt4*drhototjik2m(kl,2)
                    derv3(jz,kl) = derv3(jz,kl) + dt3*drhototijk2m(kl,3) + dt4*drhototjik2m(kl,3)
                  enddo
                else
                  dt1 = rscrhoi3*ofctijk
                  dt2 = rscrhoj3*ofctijk
...
!
                if (lMEAM) then
                  do kl = 1,nstrains
                    ...
                    do km = 1,nstrains
                      kt = nstrptr(km)
                      elcom(km,kl) = elcom(km,kl) + oij*(dt1*drhototiks(ks)*drhototijs(kt) + dt2*drhototjks(ks)*drhototjis(kt))
                      elcom(km,kl) = elcom(km,kl) + oij*(dt3*drhototijk2s(kt,ks) + dt4*drhototjik2s(kt,ks))
                    enddo
                  enddo
                else
                  do kl = 1,nstrains
