  subroutine outsync(nrep,fcrep,gcrep)
!
!  Output final state of synchronous transit run
!
!  11/06 Created from outneb
!
!  Conditions of use:
!
...
!
  ...
  if (status/=0) call outofmemory('outsync','gradrep')
!
!  Output energies of configurations
!
  write(ioout,'(/)')
  write(ioout,'(''--------------------------------------------------------------------------------'')')
  write(ioout,'(''  Energy of configurations :'')')
  write(ioout,'(''--------------------------------------------------------------------------------'')')
!      write(ioout,'(''  Initial configuration '',f20.8,'' eV'')') fcrep(1)
  write(ioout,'(''  Bound 1 '',14x,f20.8,'' eV'')') fcrep(1)
  write(ioout,'(''  Bound 2 '',14x,f20.8,'' eV'')') fcrep(2)
!      write(ioout,'(''  Final   configuration '',f20.8,'' eV'')') fcrep(4)
  write(ioout,'(''--------------------------------------------------------------------------------'',/)')
!
!  Output replicas
!
...
!
      ...
      do n = 1,nvar  
        if (iopt(n).gt.mvar) then
          i = iopt(n) - mvar
          gradrep(4,i) = gcrep(n,nrp)
        elseif (iopt(n).gt.nstrains) then
          nn = iopt(n) - nstrains
          i = (nn - 1)/3 + 1
          ix = nn - 3*(i - 1)
          gradrep(ix,i) = gcrep(n,nrp)
        else
          cgradrep(iopt(n)) = gcrep(n,nrp)
        endif
      enddo
!
      write(ioout,'(''********************************************************************************'')')
      write(ioout,'('' Bound number = '',i4)') ii - nrfirst + 1
      write(ioout,'(''********************************************************************************'')')
      if (ndim.eq.3) then
...
!
  ...
  if (status/=0) call deallocate_error('outsync','gradrep')
!
  return
  end
