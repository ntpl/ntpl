  subroutine brennersd2(ebrenner,lgrad1,lgrad2)
!
...
!  checked during the setup phase. Version for symmetry adapted
!  calculation.
!
!  On entry : 
!
...
!   5/03 Set up of spatial decomposition moved to calling routine
!  12/03 Maxneigh can now be dynamically changed
!   6/04 Calculation of energy corrected
...
!
  ...
  if (status/=0) call outofmemory('brennersd2','nREBOatomptr')
  allocate(nREBOatomRptr(numat),stat=status)
  if (status/=0) call outofmemory('brennersd2','nREBOatomRptr')
! 
...
!
  ...
  if (status/=0) call outofmemory('brennersd2','lanyau')
  allocate(latomdone(nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennersd2','latomdone')
  allocate(lopanyneigh(nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennersd2','lopanyneigh')
  allocate(nauatom(numat),stat=status)
  if (status/=0) call outofmemory('brennersd2','nauatom')
  allocate(nfreeatom(numat),stat=status)
  if (status/=0) call outofmemory('brennersd2','nfreeatom')
  allocate(nfreeatomau(numat),stat=status)
  if (status/=0) call outofmemory('brennersd2','nfreeatomau')
  allocate(nneigh(nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennersd2','nneigh')
  allocate(nsneigh(nREBOspecies,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennersd2','nsneigh')
!     
...
100 continue                                       
  ...
  if (allocated(rneigh)) then
    ...
    if (status/=0) call deallocate_error('brennersd2','d2Ndr2')
    deallocate(d2Nconjdj2,stat=status)
    if (status/=0) call deallocate_error('brennersd2','d2Nconjdj2')
    deallocate(d2Nconjdi2,stat=status)
    if (status/=0) call deallocate_error('brennersd2','d2Nconjdi2')
    deallocate(d2Btotj,stat=status)
    if (status/=0) call deallocate_error('brennersd2','d2Btotj')
    deallocate(d2Btoti,stat=status)
    if (status/=0) call deallocate_error('brennersd2','d2Btoti')
    deallocate(d2j,stat=status)   
    if (status/=0) call deallocate_error('brennersd2','d2j')
    deallocate(d2i,stat=status)
    if (status/=0) call deallocate_error('brennersd2','d2i')
!       
    deallocate(dNdr,stat=status)
    if (status/=0) call deallocate_error('brennersd2','dNdr')
    deallocate(dNconjdj,stat=status)
    if (status/=0) call deallocate_error('brennersd2','dNconjdj')
    deallocate(dNconjdi,stat=status)
    if (status/=0) call deallocate_error('brennersd2','dNconjdi')
    deallocate(d1Btotj,stat=status)
    if (status/=0) call deallocate_error('brennersd2','d1Btotj')
    deallocate(d1Btoti,stat=status)
    if (status/=0) call deallocate_error('brennersd2','d1Btoti')
    deallocate(d1j,stat=status)
    if (status/=0) call deallocate_error('brennersd2','d1j')
    deallocate(d1i,stat=status)
    if (status/=0) call deallocate_error('brennersd2','d1i')
!
    deallocate(zneigh,stat=status)
    if (status/=0) call deallocate_error('brennersd2','zneigh')
    deallocate(yneigh,stat=status)
    if (status/=0) call deallocate_error('brennersd2','yneigh')
    deallocate(xneigh,stat=status)
    if (status/=0) call deallocate_error('brennersd2','xneigh')
    deallocate(rneigh,stat=status)
    if (status/=0) call deallocate_error('brennersd2','rneigh')
    deallocate(neighno,stat=status)
    if (status/=0) call deallocate_error('brennersd2','neighno')
    deallocate(nREBObond,stat=status)
    if (status/=0) call deallocate_error('brennersd2','nREBObond')
  endif
!
...
!
  ...
  if (status/=0) call outofmemory('brennersd2','nREBObond')
  allocate(neighno(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennersd2','neighno')
  allocate(rneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennersd2','rneigh')
  allocate(xneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennersd2','xneigh')
  allocate(yneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennersd2','yneigh')
  allocate(zneigh(maxneigh,nREBOatom),stat=status)
  if (status/=0) call outofmemory('brennersd2','zneigh')
  if (lgrad1) then
    allocate(d1i(maxneigh2),stat=status)
    if (status/=0) call outofmemory('brennersd2','d1i')
    allocate(d1j(maxneigh2),stat=status)
    if (status/=0) call outofmemory('brennersd2','d1j')
    allocate(d1Btoti(maxneigh2),stat=status)
    if (status/=0) call outofmemory('brennersd2','d1Btoti')
    allocate(d1Btotj(maxneigh2),stat=status)
    if (status/=0) call outofmemory('brennersd2','d1Btotj')
    allocate(dNconjdi(maxneigh),stat=status)
    if (status/=0) call outofmemory('brennersd2','dNconjdi')
    allocate(dNconjdj(maxneigh),stat=status)
    if (status/=0) call outofmemory('brennersd2','dNconjdj')
    allocate(dNdr(maxneigh,numat),stat=status)
    if (status/=0) call outofmemory('brennersd2','dNdr')
  else
  ...
  if (lgrad2) then
    ...
    if (status/=0) call outofmemory('brennersd2','d2i')
    allocate(d2j(maxneigh22),stat=status)   
    if (status/=0) call outofmemory('brennersd2','d2j')
    allocate(d2Btoti(maxneigh22),stat=status)
    if (status/=0) call outofmemory('brennersd2','d2Btoti')
    allocate(d2Btotj(maxneigh22),stat=status)
    if (status/=0) call outofmemory('brennersd2','d2Btotj')
    allocate(d2Nconjdi2(maxneigh*(maxneigh+1)/2),stat=status)
    if (status/=0) call outofmemory('brennersd2','d2Nconjdi2')
    allocate(d2Nconjdj2(maxneigh*(maxneigh+1)/2),stat=status)
    if (status/=0) call outofmemory('brennersd2','d2Nconjdj2')
    allocate(d2Ndr2(maxneigh,numat),stat=status)
    if (status/=0) call outofmemory('brennersd2','d2Ndr2')
  else
...
!
!  Set initial value for lopanyneigh - this variable indicates whether
!  an atom has any neighbours for which derivatives are required
!
    if (.not.lfreeze) then
...
!     
    ...
    do n = 1,nneigh(nri)
      ...
      rij = rneigh(n,nri)
!
!  Check whether neighbour is free to optimise
!
...
!
      if ((lopanyneigh(nri).or.lopanyneigh(nrj)).and.(lanyau(i).or.lanyau(j).or.lstr)) then
!
...
!
                  ...
                  d2Btoti(nn) = d2Btoti(nn) + 2.0_dp*d2GijkdrdNti(2)*dNdr(l,i)*fik*expijk
!
                  if (ni.ge.l) then
                    nn = ni*(ni - 1)/2 + l
                  else
...
!
                  ...
                  d2Btotj(nn) = d2Btotj(nn) + 2.0*d2GjikdrdNtj(2)*dNdr(l,nrj)*fjk*expjik
!
                  if (nj.ge.l) then
                    nn = nj*(nj - 1)/2 + l
                  else
                  ...
                  d2Btotj(nn) = d2Btotj(nn) + 2.0*d2GjikdrdNtj(1)*dNdr(l,nrj)*fjk*expjik
!
                  if (nik.ge.l) then
                    nn = nik*(nik - 1)/2 + l
                  else
                  ...
                  d2Btotj(nn) = d2Btotj(nn) + 2.0*d2GjikdrdNtj(3)*dNdr(l,nrj)*fjk*expjik
!
                  do m = 1,l
                    if (m.ne.nj) then
                      nn = l*(l - 1)/2 + m
      ...
      if (lgrad1) then
        do nn = 1,nneigh(nri)
          if (nn.ne.ni) then
            ...
            d1Btoti(nn) = d1Btoti(nn) + dPijdNi(ns1)*dNdr(nn,i)
          endif
        enddo
        if (lgrad2) then
          do nn1 = 1,nneigh(nri)
...
!
          ...
          do nn1 = 1,nneighi2
            do nn2 = 1,nn1
              nn = nn + 1
              d2Btoti(nn) = 0.5_dp*bdelta(nREBOsi)*(bdelta(nREBOsi) + 1.0_dp)*d1Btoti(nn2)*d1Btoti(nn1)*bij/ &
                (bijsum**2) + d2Btoti(nn)
            enddo
          enddo
          do nn = 1,nneighj22
          ...
          do nn1 = 1,nneighj2
            do nn2 = 1,nn1
              nn = nn + 1
              d2Btotj(nn) = 0.5_dp*bdelta(nREBOsj)*(bdelta(nREBOsj) + 1.0_dp)*d1Btotj(nn2)*d1Btotj(nn1)*bji/ &
                (bjisum**2) + d2Btotj(nn)
            enddo
          enddo
        endif
...
!
      if (lgrad1) then
        call d1adds(j,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,nauatom,neqv,d1j,.true.)
        if (lgrad2) then
          call d2adds(j,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nfreeatomau,nREBOatomRptr,nauatom, &
                      neqv,d1j,d2j,.true.)
        endif
      endif
!
...
!
    if (lgrad1) then
      call d1adds(i,maxneigh,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nREBOatomRptr,nauatom,neqv,d1i,.true.)
      if (lgrad2) then
        call d2adds(i,maxneigh,nneigh,neighno,xneigh,yneigh,zneigh,nfreeatom,nfreeatomau,nREBOatomRptr,nauatom, &
                    neqv,d1i,d2i,.true.)
      endif
    endif
  enddo
...
!
  ...
  if (status/=0) call deallocate_error('brennersd2','nsneigh')
  deallocate(nneigh,stat=status)
  if (status/=0) call deallocate_error('brennersd2','nneigh')
  deallocate(nfreeatomau,stat=status)
  if (status/=0) call deallocate_error('brennersd2','nfreeatomau')
  deallocate(nfreeatom,stat=status)
  if (status/=0) call deallocate_error('brennersd2','nfreeatom')
  deallocate(nauatom,stat=status)
  if (status/=0) call deallocate_error('brennersd2','nauatom')
  deallocate(lopanyneigh,stat=status)
  if (status/=0) call deallocate_error('brennersd2','lopanyneigh')
  deallocate(latomdone,stat=status)
  if (status/=0) call deallocate_error('brennersd2','latomdone')
  deallocate(lanyau,stat=status)
  if (status/=0) call deallocate_error('brennersd2','lanyau')
  deallocate(nREBOatomRptr,stat=status)
  if (status/=0) call deallocate_error('brennersd2','nREBOatomRptr')
  deallocate(nREBOatomptr,stat=status)
  if (status/=0) call deallocate_error('brennersd2','nREBOatomptr')
!
  t2 = cputime()
