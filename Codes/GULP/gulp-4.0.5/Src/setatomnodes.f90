  subroutine setatomnodes(numat,nprocs,procid,lspatial)
!
!  Sets up the distribution of atoms over nodes for a 
!  spatial decomposition or otherwise in parallel.
!
!   1/03 Created
!   3/04 Modified to avoid large numbers of memory reallocations in parallel
...
!
    ...
    if (status/=0) call outofmemory('setatomnodes','npgridxptr')
    allocate(npgridyptr(0:npgrid(2)),stat=status)
    if (status/=0) call outofmemory('setatomnodes','npgridyptr')
    allocate(npgridzptr(0:npgrid(3)),stat=status)
    if (status/=0) call outofmemory('setatomnodes','npgridzptr')
!
...
!
    ...
    if (status/=0) call deallocate_error('setatomnodes','npgridzptr')
    deallocate(npgridyptr,stat=status)
    if (status/=0) call deallocate_error('setatomnodes','npgridyptr')
    deallocate(npgridxptr,stat=status)
    if (status/=0) call deallocate_error('setatomnodes','npgridxptr')
  endif
!
