  subroutine outmarv2(iout)
!
!  Write out marvin2 input file
!
!   2/02 Created from outmarv
!   2/06 Rmin added for Morse potential
...
!************************
        do i = 1,numat
          ...
          yfi = yfrac(i)
          zfi = zfrac(i)
          xfi = mod(xfi+1.0d0,1.0d0)
          yfi = mod(yfi+1.0d0,1.0d0)
          zfi = mod(zfi+1.0d0,1.0d0)
...
!
        ...
        if (mmexc(i).gt.0) then
          write(iout,'(2(a5,1x,a4,1x))') &
            lab1,stype(npt1),lab2,stype(npt2)
        else
