...
static PyMethodDef functions[] = {
  ...
  {"primitive", find_primitive, METH_VARARGS, "Find primitive cell in the input cell"},
  ...
  {"triplets_reciprocal_mesh_at_q", get_triplets_reciprocal_mesh_at_q, METH_VARARGS, "Triplets on reciprocal mesh points at a specific q-point"},
  {"triplets_reciprocal_mesh_at_q_from_triplets", extract_triplets_reciprocal_mesh_at_q, METH_VARARGS, "Triplets on reciprocal mesh points at a specific q-point extracted from full triplets"},
  {NULL, NULL, 0, NULL}
};
...
static PyObject * get_spacegroup(PyObject *self, PyObject *args)
{
  ...
  const int num_spg = spg_get_international( symbol,
					     lat,
					     pos,
					     typat,
					     num_atom,
...
static PyObject * refine_cell(PyObject *self, PyObject *args)
{
  ...
  PyArrayObject* position;
  PyArrayObject* atom_type;
  if (!PyArg_ParseTuple(args, "OOOid", &lattice, &position, &atom_type, &num_atom, &symprec)) {
    return NULL;
  }
  ...
  int num_atom_brv = spg_refine_cell( lat,
				      pos,
				      typat,
				      num_atom,
				      symprec );
...
static PyObject * get_multiplicity(PyObject *self, PyObject *args)
{
  ...
  const int num_sym = spg_get_multiplicity( lat,
					    pos,
					    types,
					    num_atom,
					    symprec );
...
static PyObject * get_symmetry(PyObject *self, PyObject *args)
{
  ...
  const int num_sym = spg_get_symmetry( rot,
					trans,
...
static PyObject * extract_triplets_reciprocal_mesh_at_q(PyObject *self, PyObject *args)
{
  ...
  PyArrayObject* rotations;
  double symprec;
  if (!PyArg_ParseTuple( args, "OOiOOOiOOd",
			 &triplets_at_q,
  ...
  const long *weight_triplets_long = (long*)weight_triplets->data;
  int weight_triplets_int[num_triplets];

  for ( i = 0; i < num_triplets; i++ ) {
    weight_triplets_int[i] = (int) weight_triplets_long[i];
    for ( j = 0; j < 3; j++ ) {
